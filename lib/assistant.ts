...
import { STORAGE_KEYS, GREETINGS, ASSISTANT_NAME } from './config';
import {
  getLiturgicalSeason, getLiturgicalSeasonLabel, checkFeastDay,
  getRandomVerseForToday, getContextualVerse, EMOTION_THEME_MAP,
...
function getBibleLang(): BibleLanguage {
  const sett = Storage.load<{ bibleLanguage?: BibleLanguage }>(STORAGE_KEYS.SETTINGS);
...
export function getDailyBrief(): string {
  const now = new Date();
  const appointments = Storage.load<Appointment[]>(STORAGE_KEYS.APPOINTMENTS) ?? [];
  const reminders = Storage.load<Reminder[]>(STORAGE_KEYS.REMINDERS) ?? [];
  const profile = Storage.loadSecure<UserProfile>(STORAGE_KEYS.USER_PROFILE);
  ...
  const inboxBrief = getInboxBrief();
...
export function saveChatMessage(message: ChatMessage): void {
  const history = Storage.load<ChatMessage[]>(STORAGE_KEYS.CHAT_HISTORY) ?? [];
  history.push(message);
  Storage.save(STORAGE_KEYS.CHAT_HISTORY, history.slice(-100));
}

export function getChatHistory(): ChatMessage[] {
  return Storage.load<ChatMessage[]>(STORAGE_KEYS.CHAT_HISTORY) ?? [];
}
